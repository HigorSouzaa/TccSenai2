...
export default function DietaGerada() {
  const navigation = useNavigation();
  const route = useRoute();
  const { userData, dataResposta } = route.params;
  ...
  useEffect(() => {
    console.log("Dados da resposta da API:", dataResposta);
  }, [dataResposta]);



  if (!fontsLoaded || !dataResposta) {
    return null;
  }
      ...
      <ScrollView>
        <View style={styles.container}>
          ...
          <View style={styles.conteiner_body}>
            {dataResposta.dietaDoDia.map((refeicao, index) => (
          ...
          <View style={styles.box_observacao}>
            ...
            <Text style={styles.txt_p_body}>{dataResposta.observacao}</Text>
          </View>
